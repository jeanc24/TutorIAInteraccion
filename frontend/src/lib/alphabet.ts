...
export const ALPHABET_MAP = new Map<string, LetterData>(
  ALPHABET.map((l) => [l.letter, l])
);

export const TOTAL_LETTERS = ALPHABET.length;

export function getLetterByIndex(index: number): LetterData {
  return ALPHABET[((index % TOTAL_LETTERS) + TOTAL_LETTERS) % TOTAL_LETTERS];
}

export function getLetterIndex(letter: string): number {
  return ALPHABET.findIndex((l) => l.letter === letter);
}

export function getNextLetter(current: string): LetterData {
  const idx = getLetterIndex(current);
  return getLetterByIndex(idx + 1);
}

export function getPrevLetter(current: string): LetterData {
  const idx = getLetterIndex(current);
  return getLetterByIndex(idx - 1);
}
...
export function getLetterImagePath(letter: string): string {
  ...
  if (l === 'Ñ') return '/alphabet/enie.png';
  return `/alphabet/${l.toLowerCase()}.png`;
}
